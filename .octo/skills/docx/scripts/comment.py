...
def _append_xml(xml_path: Path, root_tag: str, content: str) -> None:
    ...
    if len(roots) == 0:
        print(f"Error: Root tag <{root_tag}> not found in {xml_path}", file=sys.stderr)
    ...
    try:
        ...
        print(f"Error: Failed to parse content XML for {xml_path}: {e}", file=sys.stderr)
        raise
...
def _find_para_id(comments_path: Path, comment_id: int) -> str | None:
    try:
        ...
        print(f"Error: Failed to parse {comments_path}: {e}", file=sys.stderr)
        return None
...
def _get_next_rid(rels_path: Path) -> int:
    try:
        ...
        print(f"Error: Failed to parse {rels_path}: {e}", file=sys.stderr)
        raise
    max_rid = 0
...
def _has_content_type(ct_path: Path, part_name: str) -> bool:
    try:
        ...
        print(f"Error: Failed to parse {ct_path}: {e}", file=sys.stderr)
        return False
...
def add_comment(
    ...
    if first_comment:
        try:
            ...
            return "", f"Error: Failed to copy comments template to {comments}: {e}"
    ...
    if not ext.exists():
        try:
            ...
            return "", f"Error: Failed to copy commentsExtended template to {ext}: {e}"
    ...
    if not ids.exists():
        try:
            ...
            return "", f"Error: Failed to copy commentsIds template to {ids}: {e}"
    _append_xml(
        ids,
        "w16cid:commentsIds",
    ...
    if not extensible.exists():
        try:
            ...
            return "", f"Error: Failed to copy commentsExtensible template to {extensible}: {e}"
    _append_xml(
        extensible,
