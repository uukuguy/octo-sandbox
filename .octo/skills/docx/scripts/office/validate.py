...
def main():
    ...
    if args.original:
        ...
        if original_file.suffix.lower() not in [".docx", ".pptx", ".xlsx"]:
            raise ValueError(f"Error: {original_file} must be a .docx, .pptx, or .xlsx file")
    ...
    if path.is_file() and path.suffix.lower() in [".docx", ".pptx", ".xlsx"]:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            _run_validators(unpacked_dir, original_file, file_extension, args)
    else:
        if not path.is_dir():
...
def _run_validators(unpacked_dir: Path, original_file, file_extension: str, args):
