...
_SHIM_SO = Path(tempfile.gettempdir()) / f"lo_socket_shim_{os.getpid()}.so"
...
def _ensure_shim() -> Path:
    ...
    src = Path(tempfile.gettempdir()) / f"lo_socket_shim_{os.getpid()}.c"
    ...
    try:
        ...
        print(
            "ERROR: gcc compilation timed out after 60s.",
            file=sys.stderr,
        )
        raise
    except FileNotFoundError:
        print(
            "ERROR: gcc not found. Cannot compile the LibreOffice socket shim.\n"
        ...
        print(
            f"ERROR: gcc failed to compile the LibreOffice socket shim.\n"
