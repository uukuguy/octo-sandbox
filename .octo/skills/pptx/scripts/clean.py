...
def get_slides_in_sldidlst(unpacked_dir: Path) -> set[str]:
    ...
    try:
        ...
        print(f"Warning: Failed to parse {pres_rels_path}: {e}", file=sys.stderr)
        return set()
    rid_to_slide = {}
    ...
    for slide_file in slides_dir.glob("slide*.xml"):
        if slide_file.name not in referenced_slides:
            ...
            try:
                ...
                print(f"Warning: Failed to remove {slide_file}: {e}", file=sys.stderr)
                continue
            ...
            if rels_file.exists():
                try:
                    ...
                    print(f"Warning: Failed to remove {rels_file}: {e}", file=sys.stderr)
                    continue
...
def remove_trash_directory(unpacked_dir: Path) -> list[str]:
    ...
    if trash_dir.exists() and trash_dir.is_dir():
        ...
        try:
            ...
            print(f"Warning: Failed to remove directory {trash_dir}: {e}", file=sys.stderr)
...
def get_slide_referenced_files(unpacked_dir: Path) -> set:
    ...
    for rels_file in slides_rels_dir.glob("*.rels"):
        try:
            ...
            print(f"Warning: Failed to parse {rels_file}: {e}", file=sys.stderr)
            continue
            ...
            if rel_path not in referenced:
                ...
                if theme_rels.exists():
                    try:
                        ...
                        print(f"Warning: Failed to remove {theme_rels}: {e}", file=sys.stderr)
                        continue
...
def update_content_types(unpacked_dir: Path, removed_files: list[str]) -> None:
    ...
    try:
        ...
        print(f"Warning: Failed to parse {ct_path}: {e}", file=sys.stderr)
        return
    changed = False
