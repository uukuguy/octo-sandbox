...
<p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
  ...
  <p:clrMapOvr>
    ...
    try:
        ...
        print(f"Error: Failed to write slide file {dest_slide}: {e}", file=sys.stderr)
        sys.exit(1)
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/{layout_file}"/>
    ...
    try:
        ...
        print(f"Error: Failed to write rels file {dest_rels}: {e}", file=sys.stderr)
        sys.exit(1)
...
def duplicate_slide(unpacked_dir: Path, source: str) -> None:
    ...
    try:
        ...
        print(f"Error: Failed to copy slide {source_slide}: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    if source_rels.exists():
        try:
            ...
            print(f"Error: Failed to copy rels {source_rels}: {e}", file=sys.stderr)
            sys.exit(1)

        try:
            rels_content = dest_rels.read_text(encoding="utf-8")
        except OSError as e:
            print(f"Error: Failed to read {dest_rels}: {e}", file=sys.stderr)
            sys.exit(1)
        rels_content = re.sub(
        ...
        try:
            ...
            print(f"Error: Failed to write {dest_rels}: {e}", file=sys.stderr)
            sys.exit(1)
...
def _add_to_content_types(unpacked_dir: Path, dest: str) -> None:
    ...
    try:
        ...
        print(f"Error: Failed to read {content_types_path}: {e}", file=sys.stderr)
        return
    ...
    if f"/ppt/slides/{dest}" not in content_types:
        ...
        try:
            ...
            print(f"Error: Failed to write {content_types_path}: {e}", file=sys.stderr)
...
def _add_to_presentation_rels(unpacked_dir: Path, dest: str) -> str:
    ...
    try:
        ...
        print(f"Error: Failed to read {pres_rels_path}: {e}", file=sys.stderr)
        return "rId1"
    ...
    if f"slides/{dest}" not in pres_rels:
        ...
        try:
            ...
            print(f"Error: Failed to write {pres_rels_path}: {e}", file=sys.stderr)
...
def _get_next_slide_id(unpacked_dir: Path) -> int:
    ...
    try:
        ...
        print(f"Error: Failed to read {pres_path}: {e}", file=sys.stderr)
        return 256
