...
| ID | 内容 | 前置条件 | 状态 |
|----|------|---------|------|
| AP-D1 | 工具结果外部持久化（recordContentReplacement） | Session 文件架构设计 | ✅ 已补 (AQ-T3 BlobStore) |
...
| AP-D9 | 自主模式 Phase 2（Webhook/Cron 触发 + 暂停恢复 API） | T14 完成 | ✅ 已补 (AQ-T4+T5 tick loop + pause/resume) |
| AP-D10 | 自主模式 Phase 3（用户感知 + 审计日志） | AP-D9 完成 | ✅ 已补 (AQ-T6 audit + user presence) |
...
| AP-D13 | tool_search 工具（大量 MCP 工具时的搜索） | 按需 | ✅ 已补 (AQ-T2 ToolSearchTool) |
| AP-D14 | ask_user 工具（结构化提问） | ApprovalGate 扩展 | ✅ 已补 (AQ-T1 InteractionGate + AskUserTool) |
