...
| v1.2 | 2026-03-03 | D3 设计完成：新增 AgentRuntime 持久化主循环架构（3.2.2 节），详见 AGENT_RUNTIME_DESIGN.md |
...
## 3.2.2 AgentRuntime — 持久化 Agent 主循环（Phase D3）

**变更记录**：2026-03-03，D3 设计完成，详见 [AGENT_RUNTIME_DESIGN.md](AGENT_RUNTIME_DESIGN.md)

octo-sandbox 在 D3 阶段引入 `AgentRuntime`，将 Agent 从"无状态按需创建"升级为"持久化自主智能体实体"。

### 核心架构变化

```
【旧架构 - 无状态】                    【新架构 - 持久化实体】
WebSocket 收到消息                      WebSocket 收到消息
  → AgentLoop::new()  ← 每次新建          → AgentRuntimeRegistry.get_or_spawn()
  → run(load from DB) ← 每次重建历史        → AgentRuntimeHandle.send(msg)
  → 销毁                                   → 订阅广播等结果
                                         ↓
                                       AgentRuntime（持久化 tokio task）
                                         while rx.recv():
                                           history += user_msg  ← 内存积累
                                           AgentLoop.run(&mut history)
                                           SessionStore.set_messages()  ← DB持久化
```
...
| 组件 | 职责 |
|------|------|
| `AgentRuntime` | Agent 自主智能体本体：持久化主循环、Harness 执行、历史积累 |
| `AgentRuntimeRegistry` | 生命周期管理：Session → Handle 映射，spawn / get / remove |
| `AgentRuntimeHandle` | 对外句柄（可 clone）：mpsc::Sender + broadcast::Sender |
| `AgentRunner` | 依赖注入容器：提供 provider / memory / build_tool_registry |
| `AgentLoop` | 无状态执行器：单 round LLM + Tool 调用链（不变） |
| `ws.rs` | 纯 Channel 层：路由消息到 AgentRuntime，订阅广播 |

### 多 Channel 广播设计

单 Agent 多 WebSocket 连接场景（PC + 移动端等），所有连接订阅同一个 `broadcast::Sender<AgentEvent>`，看到完整共享上下文。

```
AgentRuntime.broadcast_tx
  ├── WebSocket conn A → subscribe() → Receiver A
  ├── WebSocket conn B → subscribe() → Receiver B
  └── （未来）Telegram  → subscribe() → Receiver C
```
