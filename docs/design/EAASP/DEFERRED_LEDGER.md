...
**最后更新**: 2026-04-16 (Phase 2.5 W1.T2 — GooseAdapter subprocess+ACP lifecycle; D141 新增 via W1.T2 F1 gate defer)
...
| **D141** | Phase 2.5 W1.T2 — F1 gate: goose extensions config middleware insertion runtime verification | 🟡 P1-defer | **Filed**: 2026-04-16 during W1.T2 implementation. **Scope**: goose binary not installed on local dev; ACP `--stdio` flag + `~/.config/goose/config.yaml` extensions format unverified at runtime. **Impact**: W1.T2 code written per Block's published ACP docs but not runtime-validated; W1.T3 middleware wiring + W1.T4 contract tests require goose binary on CI runner. **Resolution path**: install goose CLI on CI runner (or dev machine) → run `goose info` + `goose acp --help` → adjust CLI invocation in `crates/eaasp-goose-runtime/src/goose_adapter.rs` line ~50 if flag differs → remove skip guards in `crates/eaasp-goose-runtime/tests/adapter_test.rs` when all tests green. **Blocks**: W1.T3 real middleware wiring (contract tests assume adapter works); W1.T4 contract v1 GREEN; W1.T5 E2E smoke. **Owner**: Phase 2.5 CI setup task or developer installing goose for T3 implementation. 衍生自 Phase 2.5 W1.T2 |
