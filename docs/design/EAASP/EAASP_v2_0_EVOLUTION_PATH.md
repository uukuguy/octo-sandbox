...
| **Phase 0.75** | **L2 MCP 编排与部署架构** | **圈 2 补强** | L2 MCP Orchestrator 职责定义 + MCP transport 统一策略 + 容器化 runtime MCP 发现机制 + claude-code-runtime MCP 通路修复 | 三个 runtime 的 MCP tool call 全部通过真实 MCP server（非 workaround）；`eaasp-cli session send` 在 claude-code-runtime 上调用 mock-scada 成功 | 🟡 MCP tool call 端到端未通 |
