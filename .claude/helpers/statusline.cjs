...
// V3 progress from metrics files + actual DDD documents (pure file reads)
function getV3Progress() {
  ...
  // Try to read actual DDD document for real domain count
  const settings = getSettings();
  const configuredDddDir = settings && settings.claudeFlow && settings.claudeFlow.ddd
    ? settings.claudeFlow.ddd.directory : null;
  const dddCandidates = [
    configuredDddDir ? path.join(CWD, configuredDddDir.replace(/^\//, '')) : null,
    path.join(CWD, 'docs', 'ddd'),
    path.join(CWD, 'docs', 'design'),
  ].filter(Boolean);
  const dddPaths = [...new Set(dddCandidates.map(p => path.resolve(p)))];

  for (const dddPath of dddPaths) {
    try {
      if (!fs.existsSync(dddPath)) continue;
      const files = fs.readdirSync(dddPath).filter(f =>
        f.endsWith('.md') && (f.includes('DDD') || f.includes('ddd') || f.includes('DOMAIN'))
      );
      if (files.length > 0) {
        // Count bounded contexts by scanning for section headers
        for (const file of files) {
          try {
            const content = fs.readFileSync(path.join(dddPath, file), 'utf-8');
            // Count bounded context sections (e.g. "#### 1.1.1 Agent 执行上下文")
            // Only match section 1.x.x (actual bounded contexts), not refactoring proposals
            const bcHeaders = content.match(/^#{3,4}\s+1\.\d+\.\d+\s+/gm);
            if (bcHeaders && bcHeaders.length > 0) {
              domainsCompleted = bcHeaders.length;
              totalDomains = domainsCompleted; // Actual count is the total
              dddProgress = 100;
              break;
            }
          } catch { /* ignore */ }
        }
        if (domainsCompleted > 0) break;
      }
    } catch { /* ignore */ }
  }

  // Fallback to metrics file or pattern-based estimate
  if (domainsCompleted === 0 && dddData) {
    domainsCompleted = Math.min(totalDomains, Math.floor(dddProgress / 20));
  }
  if (domainsCompleted === 0 && learning.patterns > 0) {
...
function getADRStatus() {
  ...
  // Fallback: count ADR files from known directories (don't read them)
  // Check settings.json for configured ADR directory
  const settings = getSettings();
  const configuredAdrDir = settings && settings.claudeFlow && settings.claudeFlow.adr
    ? settings.claudeFlow.adr.directory : null;

  const candidates = [
    configuredAdrDir ? path.join(CWD, configuredAdrDir.replace(/^\//, '')) : null,
  ...
  const adrPaths = [...new Set(candidates.map(p => path.resolve(p)))];

  // Read ADR naming config from settings
  const adrConfig = settings && settings.claudeFlow && settings.claudeFlow.adr || {};
  const fileGlob = adrConfig.filePattern || 'ADR_*.md'; // e.g. "ADR_*.md"
  const filePrefix = fileGlob.replace(/[*?].*/g, ''); // e.g. "ADR_"
  const sectionRegex = adrConfig.sectionPattern
    ? new RegExp(adrConfig.sectionPattern, 'gm')
    : /^##\s+ADR-\d+/gm;
    ...
    try {
      if (fs.existsSync(adrPath)) {
        const files = fs.readdirSync(adrPath).filter(f =>
          f.endsWith('.md') && (f.startsWith(filePrefix) || f.startsWith('adr') || /^\d{4}-/.test(f))
        );
        // Count individual ADR entries: either separate files or sections within a single file
        for (const file of files) {
          try {
            const content = fs.readFileSync(path.join(adrPath, file), 'utf-8');
            const adrHeaders = content.match(sectionRegex);
            totalAdrs += adrHeaders ? adrHeaders.length : 1;
          } catch { totalAdrs += 1; }
        }
      }
    } catch { /* ignore */ }
  ...
  const implemented = totalAdrs; // All committed ADRs are considered implemented
  const compliance = totalAdrs > 0 ? 100 : 0;
  return { count: totalAdrs, implemented, compliance };
}
